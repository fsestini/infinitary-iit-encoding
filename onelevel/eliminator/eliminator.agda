...
module eliminator.eliminator where
...
module _
  ...
  --
  -- Agda seems to struggle if we put these hypotheses as module parameters,
  -- so we equivalently declare them postulates instead.
  postulate
  ...
  exists-ty-≡ {Δ = Δ} w ab@(Π A B) {p₀} p₁ cs {a₀} a₁ =
    subst-to-≅ T pf (trans (fun-subst {A = El A} _ pf f)
